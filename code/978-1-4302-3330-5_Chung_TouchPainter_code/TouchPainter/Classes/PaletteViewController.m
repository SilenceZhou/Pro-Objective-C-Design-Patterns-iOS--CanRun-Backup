...
- (void)viewDidLoad 
{
  ...
  // 用 SetStrokeColorCommand 来初始化RGB滑动条  --- 太神奇了吧  --应该是获取对象
  ...
  SetStrokeColorCommand *colorCommand = (SetStrokeColorCommand *)[redSlider_ command];
    ...
    // 把个个颜色分的提供者设置给颜色命令 （其实就是把Slider的值绑定到Block上）
...
- (IBAction) onCommandSliderValueChanged:(CommandSlider *)slider
{
    ...
    // 如果是色值slider 因为在xib把object关联为SetStrokeColorCommand 所以最终调用的是SetStrokeColorCommand中的对象方法
    
    // 如果是sizeSlider因为在Xib的object关联为 SetStrokeSizeCommand 所以最终调用的是SetStrokeSizeCommand中的对象方法
